...
stringr::str_extract(counts, "\\d{1,3}(?:[,.]\\d{3})*(?:\\.\\d+)?")
```
...
institution <- str_extract_all(
  publications_txt,
  "(University of [\\w\\s]+)|(\\w+\\s+Institute of [\\w\\s]+)"
  ) 
...
has_abstract <- sapply(pub_char_list, function(x) str_detect(x, "<AbstractText>"))

num_without_abstract <- sum(!has_abstract)

print(num_without_abstract)
...
has_title <- sapply(pub_char_list, function(x) str_detect(x, "<ArticleTitle>"))

num_without_title <- sum(has_title)

print(num_without_title)
```
...
https://htmlpreview.github.io/?https://github.com/Ekconick/lab5/blob/main/06-lab.html
